...
function EditProfile({ profileData, updateData, navigateTo }) {
	...
	const mapOptionsGeneral = optionType => {
		return options[optionType].map(x => (
			<option value={x}>{x.slice(0, 1).toUpperCase() + x.slice(1)}</option>
		));
	};
			...
			<div className="personalInformationDiv">
				...
				<div id="selectDiv">
					<span className="selectSpan">
						...
						>
							<option value="" disabled selected>
								{dataObject.gender?.slice(0, 1).toUpperCase() +
									dataObject.gender?.slice(1)}{" "}
							</option>
					...
					<span className="selectSpan">
						...
						>
							<option value="" disabled selected>
								{dataObject.nationality?.slice(0, 1).toUpperCase() +
									dataObject.nationality?.slice(1)}
							</option>
					...
					<span className="selectSpan">
						...
						>
							<option value="" disabled selected>
								{dataObject.personalityType?.slice(0, 1).toUpperCase() +
									dataObject.personalityType?.slice(1)}
							</option>
	...
	const schoolQualificationsTableRows = () => {
		return profileData.personalStory?.schoolQualifications.map((x, index) => (
			<tr>
				<td>
				...
				<table style={{ border: "1px solid black" }}>
					<thead>
						<tr>
							...
							<th style={{ backgroundColor: "#061450", color: "#fff" }}></th>
						</tr>
					</thead>
					<tbody>
						<tr>
							...
							<td>
								<button type="submit" style={{ width: "54px" }} name="update">
									Add
								</button>
							</td>
						</tr>
	...
	return (
		<>
			...
			<form onSubmit={onSubmit2} className="myForm">
				<div className="personalStory">{schoolQualificationsTable()}</div>
			</form>
		</>
	);
}
